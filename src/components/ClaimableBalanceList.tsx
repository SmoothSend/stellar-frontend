...
export function ClaimableBalanceList({ address }: ClaimableBalanceListProps) {
    ...
    useEffect(() => {
        if (address) {
            loadBalances();
            // Poll for new claims every 10 seconds? Or just manual refresh.
            // For MVP, manual refresh or one-time load is fine, but auto-refresh is nicer.
    ...
    const handleClaim = async (balance: ClaimableBalance) => {
        ...
        try {
            toast({ title: "Claiming Asset", description: `Claiming ${balance.amount} ${balance.asset}...` });
            ...
            if (result.success && result.hash) {
                toast({
                    title: "Success!",
                    description: `Successfully claimed ${balance.amount} ${balance.asset}.`,
    ...
    if (loading && balances.length === 0) {
        return <div className="text-center p-4 text-muted-foreground">Loading pending claims...</div>;
    }

    if (balances.length === 0) {
        return null; // Don't show anything if no claims (cleaner UI)
    }

    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider px-1">Pending Payments</h3>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div className="text-xs text-muted-foreground">
                                    From: {balance.sponsor.slice(0, 4)}...{balance.sponsor.slice(-4)}
                                </div>
                            </div>
                        </div>
                        <Button
                            ...
                            disabled={!!claimingId}
                            size="sm"
                            className="phantom-button h-9 px-4 font-medium"
                        >
